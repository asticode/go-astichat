...
// NewPublicKeyFromRSAPublicKey creates a PublicKey based on a *rsa.PublicKey
func NewPublicKeyFromRSAPublicKey(pub *rsa.PublicKey) PublicKey {
	return PublicKey{key: pub}
}

// NewPublicKeyFromRSAPrivateKey creates a PublicKey based on a *rsa.PrivateKey
func NewPublicKeyFromRSAPrivateKey(prv *rsa.PrivateKey) (o PublicKey, err error) {
	// Assert public key
	var pub *rsa.PublicKey
	var ok bool
	if pub, ok = prv.Public().(*rsa.PublicKey); !ok {
		err = errors.New("Public key is not a *rsa.PublicKey")
		return
	}

	// Return
	o = NewPublicKeyFromRSAPublicKey(pub)
	return
}
